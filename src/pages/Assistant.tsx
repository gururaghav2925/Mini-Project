...
type Message = {
  ...
  timestamp: Date;
  isError?: boolean;
  ingredients?: string[]; // New field to store detected ingredients
  addedToPantry?: boolean; // Track if button was clicked
};
...
export default function Assistant() {
  ...
  // --- Logic to Add Items to DB ---
  const addToPantry = async (messageId: string, ingredients: string[]) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;

      // Prepare data for bulk insert
      const itemsToInsert = ingredients.map(name => ({
        user_id: user.id,
        name: name.trim(),
  ...
  const handleSend = async (text: string = input) => {
    ...
    try {
      ...
      // --- PARSING LOGIC ---
      // Look for the "+++INGREDIENTS: ... +++" tag
      let cleanText = data.reply;
      let detectedIngredients: string[] | undefined = undefined;

      const parts = data.reply.split("+++INGREDIENTS:");
      if (parts.length > 1) {
        cleanText = parts[0].trim(); // The chat part
        const rawList = parts[1].split("+++")[0]; // The ingredients part
        detectedIngredients = rawList.split(",").map((s: string) => s.trim());
      }
      ...
      const aiMsg: Message = {
        ...
        text: cleanText,
        timestamp: new Date(),
        ingredients: detectedIngredients, // Attach list to message
      };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">AI Kitchen Assistant</h1>
            <div className="flex items-center gap-2"><span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span></span><span className="text-xs text-gray-500 font-medium">Powered by Gemini</span></div>
          </div>
        </div>
        <button onClick={() => setMessages([messages[0]])} className="text-gray-400 hover:text-gray-600 p-2 rounded-full hover:bg-gray-100 transition"><RotateCcw className="w-5 h-5" /></button>
      </div>
            ...
            <div className={`max-w-[85%] flex flex-col items-start`}>
              ...
              {msg.ingredients && msg.ingredients.length > 0 && (
                <div className="mt-2 ml-1">
                  {msg.addedToPantry ? (
                    <button disabled className="flex items-center gap-2 px-4 py-2 bg-green-100 text-green-700 rounded-xl text-xs font-bold">
                      <CheckCircle2 className="w-4 h-4" /> Added to Pantry
                    </button>
                  ) : (
                    <button 
                      onClick={() => addToPantry(msg.id, msg.ingredients!)}
                    ...
                    >
                      ...
                      Add {msg.ingredients.length} Items to Pantry
                    </button>
                  )}
                </div>
              )}
